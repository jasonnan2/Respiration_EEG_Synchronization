...
% Finally create a collated 5-D matrix (3 Freq x 68 roi x time x task x n subjects)for Pre and Post sessions
% Comment/Uncomment following Lines for running Pre and Post Separately
% 62-63,
...
%% loading in head map models and setting parameter needed for graphing
...
%% this section is the actual for loop to gather the Source roi data for each participant
%Sessionwise post (ins =1) and pre (ins =2)
    ...
    for subii = 1:size(files_all,1)
        ...
        for taski= 6 %1:Ntasks
            ...
            try
                ...
                % Now iterate through all the freq bands, filter, and save the activity
                % For all trials
                trialsind{1} = (trialsetind(blockind(find(adtrial(blockind) <= madRT)))); % Trial that has lower than 1 median deviation consistent trial
                faci_tilda = 1;
                trialno = size(trialsind{1},1);
                HighConsis_trialnos =  trialno;
                Percntg_HiCons_trials = (HighConsis_trialnos./Orig_Trial_Numb_Aft_Preprocs)*100

                %%% actually pick peb_cleaned (needs to be 24 channel .set
                %%% file)
                ...
                for clustype = 1:3 %pos, neg
                    ...
                    if isempty(tfinfoarr) == 0
                        ...
                        if size(tempvar,2) == 1999 % Just to check the time length segment
            ...
            save(fullfile(IndividualResults,[rawFileName, '_TwoTap_src_HighConsis.mat']),'tabledata');
        ...
        if ~isempty(tabledata2)
            ...
            Pre_TT_srcmap_HighConsis = squeeze(Pre_TT_srcemap(:,:,:,6,:)); % Get rid of the task dimension

        end

        filename_Trials = [Finalresults '/Trial_Numbers_HiCons_Info_CoCo.xlsx'];
        export_trials = cat(2,Orig_Trial_Numb_Aft_Preprocs, HighConsis_trialnos, Percntg_HiCons_trials);
        temp = array2table(export_trials, 'VariableNames',{'Orig_Trial_Numb_Aft_Preprocs', 'HighConsis_trialnos','Percntg_HiCons_trials'});
        temp2(subii,:)= temp;

    end

    temp2= addvars (temp2, Particp_ID, 'After', 'Percntg_HiCons_trials')


    writetable(temp2,filename_Trials,'Sheet','Pre_Trials');
    save(fullfile(Finalresults,['Pre_TwoTap_src_HighConsis.mat']),'Pre_TT_srcmap_HighConsis');

end



% size(Post_TT_srcmap_HighConsis)
% size(Pre_TT_srcmap_HighConsis)
